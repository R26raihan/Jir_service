...
def save_to_mysql(location: str, count: int, timestamp: str):
    try:
        ...
        if connection.is_connected():
            print(f"[DB CONNECTED] Menyimpan data ke database untuk lokasi: {location}")
            cursor = connection.cursor()
            query = """
                INSERT INTO crowd_history (location, count, timestamp)
                VALUES (%s, %s, %s)
            """
            values = (location, count, timestamp)
...
# === Endpoint GET hasil real-time semua lokasi ===
@app.get("/crowd")
async def get_all_crowd_data():
    return crowd_data

# === Endpoint GET history dari database ===
...
async def get_crowd_history():
    try:
        ...
        if connection.is_connected():
            print("[DB CONNECTED] Mengambil data history...")
            cursor = connection.cursor(dictionary=True)
            query = "SELECT * FROM crowd_history ORDER BY timestamp DESC LIMIT 100"
...
# === Endpoint GET density map base64 ===
